<template>
  <section class="admin-costs portal-module">
    <div class="portal-module__head">
      <div><span class="portal-module__eyebrow">Financeiro</span><h2>Gerenciar custos</h2><p>Registre despesas da empresa, usuarios, clientes e projetos.</p></div>
    ...
    <q-table flat bordered wrap-cells row-key="_id" class="portal-module__table" :rows="costs" :columns="columns" :loading="loading" no-data-label="Nenhum custo encontrado">
      ...
      <template #body-cell-actions="props"><q-td :props="props"><q-btn flat round dense icon="mdi-pencil" aria-label="Editar custo" @click="openEdit(props.row)" /><q-btn flat round dense color="negative" icon="mdi-delete-outline" aria-label="Excluir custo" @click="remove(props.row)" /></q-td></template>
    </q-table>
      ...
      <q-card class="portal-module__dialog admin-costs__dialog">
        ...
        <q-card-section>
          <q-form @submit.prevent="save">
            <q-select v-model="form.ownerType" outlined label="Destino do custo" :options="ownerOptions" emit-value map-options />
            <q-select v-if="form.ownerType !== 'company'" v-model="form.ownerId" outlined class="q-mt-sm" label="Usuario ou cliente" :options="targetOptions" emit-value map-options :rules="[requiredRule]" />
...
function emptyForm() { return { _id: '', ownerType: 'company', ownerId: '', title: '', status: 'active', project: '', description: '', amount: 0, date: new Date().toISOString().slice(0, 10), type: 'single', file: null, attachment: null, removeAttachment: false } }
...
async function load() {
  ...
  try {
    ...
    targets.value = [...(usersResponse.data.data || []), ...(customersResponse.data.data || [])]
...
async function save() {
  ...
  try {
    ...
    Object.entries(form.value).forEach(([key, value]) => {
      if (key !== 'file' && key !== 'attachment' && key !== 'removeAttachment' && key !== '_id' && value !== null && value !== undefined) body.append(key, value)
    })
...
@media (max-width: 760px) { .portal-module__head { align-items: flex-start; flex-direction: column; } .portal-module__primary { width: 100%; } .admin-costs__filters { grid-template-columns: 1fr; } .admin-costs__grid { grid-template-columns: 1fr; } .portal-module__table { max-width: calc(100vw - 2rem); overflow: auto; } }
</style>
